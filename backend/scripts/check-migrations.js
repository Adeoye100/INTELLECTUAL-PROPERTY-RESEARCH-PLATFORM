...
const expected = Array.from({ length: 14 }, (_, index) => String(index + 1).padStart(3, '0'));
const files = (await readdir(directory)).filter((file) => /^\d{3}_.+\.sql$/.test(file)).sort();
const prefixes = files.map((file) => file.slice(0, 3));
if (prefixes.join(',') !== expected.join(',')) throw new Error('Migration inventory must contain ordered migrations 001 through 014 exactly once.');
