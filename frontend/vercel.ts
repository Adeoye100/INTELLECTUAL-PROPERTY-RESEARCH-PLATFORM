// Vercel project compatibility for deployments whose Root Directory is `frontend`.
// The repository-root vercel.ts remains the source for repo-root deployments.
// Keep the routing and response-header policies aligned between both files.
...
export function createFrontendRootVercelConfig(environment: Environment) {
...
export const config = createFrontendRootVercelConfig(process.env);
