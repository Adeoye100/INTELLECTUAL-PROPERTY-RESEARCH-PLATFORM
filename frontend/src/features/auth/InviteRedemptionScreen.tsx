  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!result.data.user || result.data.user.identities?.length === 0) {
        setMode('signin'); setError('An account already exists. Sign in, or reset the password to accept this invitation.'); return;
      }
