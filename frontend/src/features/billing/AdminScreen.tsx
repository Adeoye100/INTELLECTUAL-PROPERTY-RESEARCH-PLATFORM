...
export function AdminScreen() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section aria-labelledby="history-title"><h2 id="history-title" className="text-xl font-bold">Recent transactions</h2>{summary?.transactions.length ? <ul className="mt-3 space-y-2">{summary.transactions.map((transaction) => <li key={transaction.id} className="flex flex-wrap justify-between gap-2 rounded border border-border bg-card text-card-foreground p-3"><span className="capitalize">{transaction.tier} · {transaction.status}</span><span>{currencyAmount(transaction)}</span><span className="font-mono text-xs">{transaction.reference}</span></li>)}</ul> : <p className="mt-2 text-text-secondary">No transactions yet.</p>}</section>
